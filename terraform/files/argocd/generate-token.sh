...
ARGO_AUTH_BEARER_TOKEN=$(curl -k --location --request POST "https://$ARGO_HOSTNAME/api/v1/session" \
...
ARGO_SERVICE_ACCOUNT_TOKEN=$(curl -k --location --request POST "https://$ARGO_HOSTNAME/api/v1/account/kerberus-dashboard/token" \
